...
export default {
  ...
  setShareInfo(param, response) {
    ...
    function shareInfoPost(msg) {
      let shareType = {'onMenuShareTimeline:ok':1, 'onMenuShareAppMessage:ok':2, 'sendAppMessage:ok':2, 'shareQQ:ok':3, 'shareQZone:ok':4}[msg] || 0;
      if (shareSource) {
        tj.send({
          production_data: {
    ...
    scriptjs('//3n.dvmama.com/wx_jssdk/jweixin-1.0.0.js', function() {
  ...
  goAuthPage(show) {
    ...
    var redirect_uri = `${location.protocol}//open.davdian.com/view/weixin_auth_redirect.html?tag=index&referrer_protocal=${encodeURIComponent(location.protocol)}&referrer_host=${encodeURIComponent(location.host)}&referrer_path=${encodeURIComponent(location.pathname)}`;
