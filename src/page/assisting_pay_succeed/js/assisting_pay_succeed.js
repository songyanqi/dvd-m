...
new Vue({
  ...
  data() {
    return {
      ...
      orderId: param.get("order_id", location.href),
      showPay: !param.get("notFirst", location.href),
      //showPay: true,
  ...
  methods: {
    ...
    saveCodeClick() {
      console.log("点击保存图片");
      if (ua.isDvdApp()) {
        nativeAncestry.savePic(this.teacherWxCode);
      }
    },
    ...
    getData() {
      ...
      $.ajax({
        cache: false,
        async: true,
        url: "/api/mg/content/course/paySuccess",
        type: "post",
        dataType: "json",
        data: encrypt.ajax({
          orderId: ts.orderId
        ...
        success(response) {
          ...
          try {
            if (response.code === 0) {
              ...
              ts.teacherWxCode = response.data.wxCard;
              ts.courseDetailUrl = response.data.seriesDetailUrl;

              console.log(this.teacherWxCode);
        ...
        error(error) {
          ...
          if (!"[[env]]") {
            console.error(
              `ajax已执行error回调方法: url=${this.url}, reason=${
                error.status
              } ${error.statusText} `
            );
            this.success(require("../json/assisting_pay_succeed.json"));
            console.warn(
              `ajax已使用mock数据: url=${
                this.url
              }, mock=assisting_pay_succeed.json`
            );
          }
        }
      });
