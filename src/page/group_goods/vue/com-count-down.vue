...
<script>
  export default {
    ...
    mounted: function () {
      this.remain_second = this.remainSecond
    ...
    methods: {
      ...
      computeRemainTime(){
        ...
        // 用毫秒计算,否则损失精度
        ts.remainMillionSecond = ts.remain_second * 1000;
        ...
        let interval = setInterval(function () {
          if (ts.remain_second) {
            if (ts.remain_second > 0) {
              ...
              if(secondDiff > 1100){
                ts.remain_second = parseInt(ts.remainMillionSecond / 1000);
              }else{
                ts.remain_second -- ;
              }

              // 计算显示数值
              let day = parseInt(ts.remain_second / oneDay);
              let hour = parseInt(ts.remain_second % oneDay / oneHour);
              let minute = parseInt(ts.remain_second % oneDay % oneHour / oneMinute);
              let second = parseInt(ts.remain_second % oneDay % oneHour % oneMinute);
