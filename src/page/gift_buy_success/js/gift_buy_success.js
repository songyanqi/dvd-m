...
new Vue({
  ...
  methods: {
    ...
    getData(){
      ...
      // 取缓存
      /*let response = cache.getItem('/[[project]]/gift_buy_success');
       if (response) {
      ...
      $.ajax({
        ...
        success(response) {
          ...
          try {
            if (response.code === 0) {
              ...
              // 设置缓存
              cache.setItem({
                key: '/[[project]]/gift_buy_success-getData',        // 缓存key
        ...
        error(error) {
          ...
          if (!'[[env]]') {
            ...
            this.success(require('../json/gift_buy_success.json'));
            console.warn(`ajax已使用mock数据: url=${this.url}, mock=gift_buy_success.json`);
          }
        }
      });
    },
