 ...
 */
import ua from '../../page/growthBox_download/module/ua';
...
function getProtocal(param = {}) {
  ...
  // 拼接唤起native协议
  let protocal = `growthBox:\/\/call.${param.host}.com?action=${param.action}&params=${encodeURIComponent(JSON.stringify(param.param))}&callback=${callbackName}&minv=${param.v}`;
  return protocal;
}
...
function innerCall(param = {}) {
  ...
  // alert("ua.getDvdAppVersion():"+ua.getDvdAppVersion()+"-----param.v:"+param.v);
  // alert(ua.compareVersion(ua.getDvdAppVersion(), param.v) >= 0)
...
const native = {
  ...
  VoiceLive: {
     ...
     * native.VoiceLive.enterOldVoiceLiveRoom({
         ...
         "smallImageUrl": "http://9i.dvmama.com/free/2016/07/01/320_320_d9c182776612fe1f3243b763dcf9a1b5.jpg",//分享显示图片
  ...
          'action' : '文字按钮需要执行的JS'
     ...
     * native.custom.share({
        ...
        desc: 'MAMA+|大V店',
        link: 'MAMA+|大V店',
        imgUrl: 'http://9i.dvmama.com/goods/1/20151017104524.png'
      })
     */
    share(param = {}) {
      // 分享信息(页面分享信息+默认分享信息)
     ...
     * native.custom.shareImg({
        show: '1',  // 1弹窗,0不弹窗
        shareType: '1',
        bigImageUrl: 'http://9i.dvmama.com/free/back_top_icon_0803.png'  // imgUrl无效,必须用bigImageUrl
      })
     */
    shareImg(param = {}) {
      // 默认参数
