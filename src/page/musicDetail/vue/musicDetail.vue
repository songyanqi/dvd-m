...
//                    alert("加载完毕");
    ...
    mounted: function () {
      ...
      this.$nextTick(function(){
        ...
        let musicList = param.get('musicList');
        if(musicList) {
          popup.loading(0);
          that.price = '0';
          that.musicList = JSON.parse(decodeURIComponent(musicList));
    ...
    methods: {
      ...
      playAudio(index){
        ...
        if (index >= 0){
          ...
          if(param.get('musicList')) {
            share.setShareInfo(that.musicList[that.musicList.length -1 - that.index].shareInfo);
          }
        } else {
          if (that.isPlay){
            that.isPlay = false
