      ...
      <a v-for = "item of list"
        ...
        <div class="good_img_container">
          <div class="img_container good_img_container">
            <img v-if = "item.goods_img" v-lazy="item.goods_img" style="display: inline;">
            <img v-if = "item.imageUrl" v-lazy="item.imageUrl" style="display: inline;">
          ...
          <div v-if = "item.sale_status" class="good_list_sell_out ng-scope">
            <span v-if = "item.sale_status == 'soldout'||item.sale_status == '售罄'" class="ng-scope">售罄</span>
        ...
        <div class="good_con" :class = "{ column3Con: column && column == '3' }">
          <!--卖点-->
          <!--<div class="sell-message">{{item.sellingPoint}}</div>-->
          <div class="good_title">
            <span class="type_box" v-if="item.attrs && item.attrs.length !==0">
              <i class="type-icon">{{item.attrs[0].attrInfo}}</i>
            </span>{{item.goods_name || item.title}}</div>
    ...
    methods:{
      ...
      imgObject: function (imgSrc) {
        return {
          ...
          loading: '//9i.dvmama.com/free/2017/08/04/davdianbg.jpg'
        }
      }
    },
    events:{
