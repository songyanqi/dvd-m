...
new Vue({
  ...
  data() {
    return {
      ...
      html: decodeURIComponent(param.get('html')),
    };
  },
  computed: {},
  beforeCreate() {
  ...
  methods: {
    ...
    getData(){
      ...
      // 取缓存
      /*let response = cache.getItem('/[[project]]/richtext');
       if (response) {
      ...
      $.ajax({
        ...
        success(response) {
          try {
            if (response.code === 0) {
              ...
              // 设置缓存
              cache.setItem({
                key: '/[[project]]/richtext-getData',        // 缓存key
        ...
        error(error) {
          ...
          if (!'[[env]]') {
            ...
            this.success(require('../json/richtext.json'));
            console.warn(`ajax已使用mock数据: url=${this.url}, mock=richtext.json`);
          }
        }
      });
    },
