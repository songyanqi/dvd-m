  ...
  if (dataVersion) {
    arrValue = [location.href.split('/').slice(0, 3).join('/'), document.cookie.split(';').filter(function(x) {
      ...
      ;
        ...
        return x.indexOf('dvdsid') > -1;
      })[0].split('=')[1] : 0, window.Units && Units.getDeviceToken && Units.getDeviceToken('deviceId'), 'json', new Date().getTime(), osv, '750_1334', JSON.parse(sessionStorage.getItem('dataVersion')) && JSON.parse(sessionStorage.getItem('dataVersion'))[window.dataVersion] ? JSON.parse(sessionStorage.getItem('dataVersion'))[window.dataVersion] : 0];
  } else {
    arrValue = [location.href.split('/').slice(0, 3).join('/'), document.cookie.split(';').filter(function(x) {
      return x.indexOf('dvdsid') > -1
      ;
