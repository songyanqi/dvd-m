...
return x.indexOf('dvdsid') > -1
;})[0].split('=')[1]:0, navigator.userAgent, 'json', new Date().getTime(), 'web_h5_*_*', '750_1334', JSON.parse(sessionStorage.getItem('dataVersion')) && JSON.parse(sessionStorage.getItem('dataVersion'))[window.dataVersion] ? JSON.parse(sessionStorage.getItem('dataVersion'))[window.dataVersion]:0]
};
...
let sortObj = (dataVersion, obj) => {
  ...
  // 测试
  // let arrValue = ['http://haba.davdian.com/', 'b63b64c250150b505ab0e8219325ef80adb73835', "", 'json', new Date().getTime(), 'web_h5_*_*', '750_1334', (JSON.parse(sessionStorage.getItem('dataVersion')) && JSON.parse(sessionStorage.getItem('dataVersion'))[window.dataVersion])?JSON.parse(sessionStorage.getItem('dataVersion'))[window.dataVersion]:0]
  // 线上
  ...
  // let arrValue = [location.host, document.cookie.split(';').filter(function(x){return x.indexOf("dvdsid")>-1})[0]?document.cookie.split(';').filter(function(x){return x.indexOf("dvdsid")>-1})[0].split("=")[1]:0, "", 'json', new Date().getTime(), osv, '750_1334', (JSON.parse(sessionStorage.getItem('dataVersion')) && JSON.parse(sessionStorage.getItem('dataVersion'))[window.dataVersion])?JSON.parse(sessionStorage.getItem('dataVersion'))[window.dataVersion]:0]
  let arrValue = [location.href.split('/').slice(0, 3).join('/'), document.cookie.split(';').filter(function(x) {
return x.indexOf('dvdsid') > -1
;})[0] ? document.cookie.split(';').filter(function(x) {
return x.indexOf('dvdsid') > -1;
})[0].split('=')[1]:0, window.Units && Units.getDeviceToken && Units.getDeviceToken('deviceId'), 'json', new Date().getTime(), osv, '750_1334', 0];
  if (obj) {
    for (let p in obj) {
      arrKey.push(p);
      arrValue.push(obj[p]);
...
let statistics = (obj = {}, callback = null) => {
  ...
  var feedList = JSON.parse(sessionStorage.getItem('feedList'));
  !feedList && (feedList = {});
...
let clickAnalysis = (item, scope, type) => {
  ...
  if (curItem.length) {
    cmdLog = curItem[0] && curItem[0].command && curItem[0].command.log;
  }
